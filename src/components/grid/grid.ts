...
export class Grid extends Component<Props, SpreadsheetChildEnv> {
  ...
  private keyDownMapping: { [key: string]: Function } = {
    ...
    "ALT+SHIFT+ARROWDOWN": () => this.processHeaderGroupingKey("down"),
     // ← THÊM CÁC DÒNG SAU:
  "CTRL+=": () => this.env.model.dispatch("ZOOM_IN", {}),
  "CTRL+PLUS": () => this.env.model.dispatch("ZOOM_IN", {}),
  "CTRL+-": () => this.env.model.dispatch("ZOOM_OUT", {}),
  "CTRL+MINUS": () => this.env.model.dispatch("ZOOM_OUT", {}),
  "CTRL+0": () => {
    const sheetId = this.env.model.getters.getActiveSheetId();

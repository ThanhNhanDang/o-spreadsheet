  ...
  drawGrid(renderingContext: GridRenderingContext, layer: LAYERS, zoom: number = 1) {
    ...
    switch (layer) {
      case LAYERS.Background:
        ...
        // Bắt đầu scale cho nội dung chính
        ctx.save();
        ctx.scale(zoom, zoom); 
        ...
        for (const { zone, rect } of this.getters.getAllActiveViewportsZonesAndRect()) {
          ctx.save();
          ctx.beginPath();
          // Lưu ý: rect ở đây trả về tọa độ chưa scale (do logic trong SheetView), 
          // nhưng vì ta đã ctx.scale() ở ngoài, nên việc vẽ bên trong sẽ tự động phóng to.
        ...
        // Vẽ Frozen Panes (cũng cần scale)
        this.drawFrozenPanes(renderingContext);
        
        ctx.restore(); // Kết thúc scale cho nội dung background
        break;

      case LAYERS.Headers:
        if (!this.getters.isDashboard()) {
          // Headers cũng cần zoom để khớp với nội dung
          ctx.save();
          ctx.scale(zoom, zoom);
